...
export const downloadRuntime = function(version, tmpFile, opts) {
  if (platform === 'win32') {
...
export const downloadWindowsNode = async function(version, tmpFile, opts) {
  if (await shouldUse7z(version)) {
    return download7z(version, tmpFile, opts)
  }

  if (shouldUseZip(version)) {
    return downloadZip(version, tmpFile, opts)
  }

  return downloadRaw(version, tmpFile, opts)
}
...
export const downloadUnixNode = async function(version, tmpFile, opts) {
  if (await shouldUseXz(version)) {
    return downloadXz(version, tmpFile, opts)
  }

  return downloadGz(version, tmpFile, opts)
}
