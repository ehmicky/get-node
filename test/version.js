...
import { NO_BIN_VERSION, LOCAL_VERSION } from './helpers/versions.js'

test('Does not work on very old versions', async (t) => {
  await t.throwsAsync(getNode(NO_BIN_VERSION))
})

test('"local" alias default to current process version', async (t) => {
  // eslint-disable-next-line fp/no-mutation
  env.TEST_HOME_DIR = '/'

  try {
    const output = getOutput()
    const { path, version } = await getNode(LOCAL_VERSION, { output, cwd: '/' })
